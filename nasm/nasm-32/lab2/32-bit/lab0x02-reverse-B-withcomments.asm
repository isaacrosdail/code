...
reverse_loop:
    ...
    ;; Strategy B: Loop to print 1 char at a time
...
reverse_buffer_loop:
    ...
    ;; line below is ALMOST possible in NASM 
    ;; unfortunately, you can't have memory location as a source, and another memory location as destination
    ;; mov [outputBuffer + edi], [inputBuffer, esi];; mov from address inputBuffer + charPosition to outputBuffer
...
end_reverse_buffer_loop:
    ...
    ;; we don't need the lines below because after the loop EDI will already have this value
    ;; mov edi, [inputByteCount]
    ;; dec edi                      ;; inputByteCount-1 in the position of the newline char
                                    ;; also the position we want it in in the output buffer
